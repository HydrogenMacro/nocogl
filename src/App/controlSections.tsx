    ...
    <ControlsSection title="General">
      <Row label="Project Name">
        <input
          ...
          onInput={(ev) => {
            appState.projectName = (ev.target as HTMLInputElement).value;
          }}
        />
      </Row>
            ...
            label={
              <input
                ...
                onInput={(ev) => {
                  ...
                  if (
                    snap.uniforms.some(([uniformName]) => val === uniformName)
                  ) {
                    alert("Uniform already exists.");
                  } else {
                    appState.uniforms[i] = [val, snap.uniforms[i][1]];
                  }
                }}
              />
            }
        ...
        return (
              ...
              <input
                ...
                onInput={(ev) => {
                  ...
                  if (
                    ...
                    appState.attributes[i] = [val, snap.attributes[i][1]];
                  }
                }}
              />
            }
          ...
          >
            <ReactCodeMirror
              ...
              onChange={(val) => (appState.attributes[i][1] = val)}
            />
          </Row>
        );
      })}
...
function Row({
  ...
  return (
    <tr className="flex justify-between w-full max-w-full overflow-x-auto">
      {label ? <td className="w-auto">{label}</td> : null}
      <td
        colSpan={label ? 1 : 2}
        className={clsx(
          "flex-1 flex",
          label ? ["w-[50%]", "max-w-[50%]"] : ["w-full", "max-w-full"]
        )}
      >
        {children}
      </td>
